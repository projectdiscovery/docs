...
The GCP org-level integration uses the Cloud Asset Inventory API (`cloudasset.googleapis.com`). This API must be enabled in the project where you created the service account and WIF pool.
...
1. **Cloud Asset API not enabled** — The Cloud Asset Inventory API must be enabled in the project where your service account and WIF pool were created. Check and enable it:
   ```bash
   # Check
   ...
   ```
   Replace `YOUR_PROJECT_NUMBER` with the numeric project number from your Workload Identity Provider path (the number between `projects/` and `/locations/`).

2. **Missing org-level IAM roles** — Ensure all four org-level roles are granted: `roles/cloudasset.viewer`, `roles/resourcemanager.organizationViewer`, `roles/resourcemanager.folderViewer`, and `roles/browser`. See the Organization-Level tab in the [Grant Permissions](#service-account-key) step for details.
