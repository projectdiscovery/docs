...
<CardGroup cols={2}>
  ...
  <Card title="Enumeration Events" icon="magnifying-glass">
  ...
  <Card title="New Asset" icon="server">
    ...
    <small>Triggered at end of enumeration or scan</small>
  </Card>
</CardGroup>
...
<Warning>
**Scan and Enumeration lifecycle events** trigger **3 separate webhook calls**: one when it starts (`running`), one when it completes (`finished` or `failed_stopped`). Only the relevant data object is populated in each webhook - the others will be `null`.
...
<Info>
**When triggered:** Immediately when a vulnerability scan begins execution
...
<Info>
**When triggered:** When a vulnerability scan completes successfully
...
<Info>
**When triggered:** When a scan fails or is manually stopped

**Type value:** `failed_stopped`
</Info>
...
<AccordionGroup>
  ...
  **Failed/Stopped Object:**
  ...
  <Tab title="Enumeration Events">
  
  Select an enumeration event to view its webhook payload structure:
  
  <AccordionGroup>
    <Accordion title="Enumeration Started" icon="magnifying-glass" defaultOpen="true">
  
<Info>
**When triggered:** When asset enumeration begins execution
...
<Info>
**When triggered:** When asset enumeration completes successfully
...
<Info>
**When triggered:** When enumeration fails or is manually stopped
...
<Info>
**When triggered:** At the end of a scan when new vulnerabilities are discovered (compared to previous scan)
...
<Info>
**When triggered:** At the end of enumeration when new assets are discovered

**Type value:** `new_asset`

**Configuration:** Can be enabled for enumerations, scans, or both based on your alerting configuration
</Info>
