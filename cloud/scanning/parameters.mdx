...
<Warning>
Scans run under a per-host time budget. A rate limit too low for the number of templates selected means not every template runs, and the exploitation templates that find Critical and High issues are the ones dropped.
