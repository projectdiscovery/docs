...
<Warning>
**A low rate limit reduces coverage, not just speed.** Scans run under a per-host time budget. If the selected templates cannot finish inside that budget at the configured rate, the scan stops at the budget and reports as **finished at 100%** rather than erroring, having run only the templates it reached. Lighter passive checks tend to run first, so a heavily throttled scan across the full template set can return passive findings only and miss vulnerabilities an earlier scan found.

**Symptom to look for:** a scan that completes normally but returns noticeably fewer findings than a previous scan of the same targets. Check the rate limit on the configuration that was applied.
</Warning>

<Tip>
**Sizing the rate limit:** for scans across the full template set, use **100 requests/second or higher** per host. A low value such as `20` is safe only for a small, targeted template selection that finishes well inside the budget. If a target genuinely cannot take the load, narrow the template selection as well as the rate, so the smaller set still completes.
</Tip>
