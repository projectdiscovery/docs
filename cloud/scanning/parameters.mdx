...
### Rate Limiting
Configure request rate limits to control scan speed and avoid overwhelming target hosts:
- Set requests per second per host
- Prevent network congestion
- Comply with target rate limits
...
<Steps>
  ...
  <Step title="Basic Configuration">
    1. Enter a descriptive name (e.g., "WP Authentication" or "Rate Limit Config")
    ...
    <img
      ...
      src="/images/Screenshot2025-01-09at3.31.10PM.png"
      alt="Rate limit configuration dialog"
    />
    <img
  ...
  <Step title="Add Parameters">
    For rate limiting:
    - Set requests per second per host (e.g., 60)
    - Leave empty for maximum speed (default)
  ...
  <Step title="Configure Application Scope">
    1. Choose when to apply this configuration:
       - Enable for all scans (global) - applies rate limiting to every scan automatically
       - Manual selection per scan - allows you to selectively apply rate limiting
    2. Set template matching criteria (optional)
    
    <Note>
    Rate limiting configurations are particularly useful when applied globally to ensure consistent, respectful scanning behavior across all your scans.
    </Note>
  </Step>
</Steps>
...
### Rate Limiting
```yaml
name: "Respectful Scanning"
type: rate_limiting
settings:
  rate_limit_per_host: 50
description: "Limits requests to 50 per second per host for respectful scanning"
scope: "global"
```
...
2. **Performance Optimization**
   ...
   - Set appropriate rate limits to balance speed and target stability
