...
Before installing pdcp-agent, you need to create a network in the ProjectDiscovery Cloud Platform. Each network represents a distinct internal environment (e.g., production, staging, office network) and generates a unique agent configuration.
...
<img
  width="800"
  src="/images/pdcp-create-network-demo.gif"
  alt="Creating a Network in PDCP"
/>
...
- Download and configure pdcp-agent
- Establish a secure connection to ProjectDiscovery Cloud Platform
- Register the agent with your network

Once the agent runs successfully, it will appear in your network dashboard with a **"Connected"** status, indicating it's ready to execute scans.

<img
  width="800"
  src="/images/pdcp-agent-setup-demo.gif"
  alt="Complete pdcp-agent Setup: Create Network, Install Agent, and Verify Connection"
/>
