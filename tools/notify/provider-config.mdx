---
title: Notify Provider Config
description: "Learn how to configure Notify's provider configuration"
sidebarTitle: Provider Config
---

The default provider config file can be created at `$HOME/.config/notify/provider-config.yaml`.

Each provider (`slack`, `discord`, etc.) is specified at the top level by name, and then can have 1 to many configurations for that probider identifed by a unique `id` field.
