...
## Installation Notes
  ...
  <!-- commit message: on my linux distro with bash shell: this command expanded variables and I had errors, but I fixedd this by fixing in the option below -->
  ```
  echo export $PATH=$PATH:$HOME/go/bin >> $home/.bashrc
  source $home/.bashrc
  ```
 
  If you have some errors - you can try this option for bash:
  ```
  echo "export PATH=\$PATH:\$HOME/go/bin" >> ~/.bashrc 
  source ~/.bashrc
  ```

  Option for fish shell:
  ```
  echo "set -x PATH \$PATH \$HOME/go/bin" >> ~/.config/fish/config.fish
