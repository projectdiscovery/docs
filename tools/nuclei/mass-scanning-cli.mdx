---
title: 'Mass Scanning with Nuclei CLI'
description: "Documentation related to Running Nuclei on More than 100 targets"
sidebarTitle: 'Mass Scanning'
---

## What is **Mass Scanning?**

Mass scanning in context of Nuclei means running Nuclei CLI on more than 100 targets. While Nuclei works out of the box for running scans on any number of targets, It is recommended to have a basic understanding of the resource requirements for running nuclei and how to properly adjust flags and options to avoid overutilization of resources and to get the best performance.
...
Nuclei is a highly concurrent tool that has major Network I/O due to nature of the tool. Hence There is a direct correlation between concurrency and Memory Usage
...
## Summary & Recommendations for Optimizing Resource Usage

Currently, There is no out-of-the-box solution to optimize nuclei automatically for mass scanning, only proper understanding of flags and options can help in optimizing nuclei for mass scanning.

In general, following are some recommendations to optimize nuclei for mass scanning:
...
- **-timeout**
    ...
    Again this flag depends on the targets you are running nuclei on and the network conditions. Setting low value might cause false negatives and setting high value might cause high memory usage and slow down the scanning process

- **-retries**
    Retries controls the maximum number of retries nuclei should do for a request (current default is 1)
    This flag is useful when you are scanning targets with unstable network conditions. Setting high value might cause high memory usage and slow down the scanning process

- **-mhe or -max-host-error**
    This flag controls the maximum number of (network type) errors to allow per host before kicking out that unresponsive host from the scan (current default is 30)

- **-nmhe or -no-max-host-error**
    This flag disables the behavior of kicking out unresponsive hosts from the scan when they reach maximum number of errors (current default is 30)
    Note: This flag directly affects speed and memory usage of nuclei since it keeps unresponsive hosts in memory and retries them
...
Building a Nuclei Automation or running recurrent scans on more than 100 targets can be a challenging task without proper and continued understanding of nuclei or experimenting with flags and options

This is where [PDCP](https://cloud.projectdiscovery.io) comes into play its a Managed service to Vulnerability Scanning and ASM With Nuclei and other projectdiscovery tools, Neverthless to say in context of Nuclei 
...
This is strictly speaking in context of Nuclei Mass Scanning but PDCP includes many other need to have features for Vulnerability Scanning and ASM like Dashboard, Integrations, Reporting, Recurring Scans and much more
